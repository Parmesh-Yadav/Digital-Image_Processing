...
def start(image):
    ...
    for i in range(h):
        for j in range(w):
            #mapping coordinates
            o_c = get_o(i, j)  # output coordinates 
            i_c = np.matmul(o_c,i_t_m)  # input coordinates of x5.bmp because O * T^-1 = I
            X = i_c[0][0]
            Y = i_c[0][1]
            ...
            if (p_v > 255):
                ...
                p_v = 0
            # print("Pixel value at", i, j, "is", p_v)
            new_image[i][j] = p_v
    cv2.imwrite('output.bmp',new_image)
